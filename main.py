...
#     current_user: Annotated[User, Depends(read_users_me)]
# ):
#     statement = select(Invoice).join(House, House.id == Invoice.hse_id).join(Property, Property.id == House.property_id).where(Property.landlord_id == current_user.id).options(selectinload(Invoice.house), selectinload(Invoice.utilities), selectinload(Invoice.tenant))
...
# @app.get("/invoices/rent/{hse_id}/all", response_model=List[Invoice])
# async def get_all_invoices_by_hse(
#     session: SessionDep,
#     current_user: Annotated[User, Depends(read_users_me)],
#     hse_id: str,
# ):
...
# @app.get("/invoices/rent/tenant/{tenant_id}/all", response_model=List[Invoice])
...
#             db_util.amount = utilities_updates[db_util.amount]
#             session.add(db_util)
...
# @app.get("/invoices/maintenance/all", response_model=List[MaintenanceBill])
# async def get_all_maintenance_bills(
#     session: SessionDep,
#     current_user: Annotated[User, Depends(read_users_me)]
# ):
#     statement = select(MaintenanceBill).join(House).join(Property).where(Property.landlord_id == current_user.id).options(selectinload(MaintenanceBill.payments), selectinload(MaintenanceBill.tenant))
    
#     mbs = session.exec(statement).all()
    
#     return mbs

# @app.get("/invoices/maintenance/{hse_id}/all", response_model=List[MaintenanceBill], dependencies=[Depends(read_users_me)])
# async def get_maintenance_bills_by_house(
#     session: SessionDep,
#     hse_id: UUID
# ):
#     hse_query = select(House).join(Property).where(House.id == hse_id).options(selectinload(House.property))
#     hse = session.exec(hse_query).first()
    
#     if not hse:
#         raise HTTPException(status_code=404, detail="House id is incorrect")
    
#     statement = select(MaintenanceBill).where(MaintenanceBill.hse_id == hse.id).options(selectinload(MaintenanceBill.payments), selectinload(MaintenanceBill.tenant))
#     result = session.exec(statement).all()
    
#     return result

# @app.get("/invoices/maintenance/tenant/{tenant_id}/all", response_model=List[MaintenanceBill], dependencies=[Depends(read_users_me)])
# async def get_maintenance_bills_by_tenant(
#     session: SessionDep,
#     tenant: Annotated[Tenant, Depends(get_single_tenant)]
# ):
#     if not tenant:
#         raise HTTPException(status_code=404, detail="Tenant could not be found")
    
#     statement = select(MaintenanceBill).where(MaintenanceBill.tenant_id == tenant.id).options(selectinload(MaintenanceBill.payments), selectinload(MaintenanceBill.tenant))
#     result = session.exec(statement).all()
...
#     current_user: Annotated[User, Depends(read_users_me)],
# ):
#     statement = select(Payment).join(Invoice).join(MaintenanceBill).join(House).join(Property).where(Property.landlord_id == current_user.id)
    
#     payments = session.exec(statement).all()
    
#     return payments

# @app.get("/payments/house/{hse_id}/all", response_model=List[Payment], dependencies=[Depends(read_users_me)])
# async def get_all_payments_by_house(
#     session: SessionDep,
#     hse_id: UUID,
# ):
#     statement = select(Payment).join(Invoice).join(MaintenanceBill).join(House).where(House.id == hse_id)
    
#     payments = session.exec(statement).all()
    
#     return payments
    
# @app.get("/payments/tenant/{tenant_id}/all", response_model=List[Payment], dependencies=[Depends(read_users_me)])
# async def get_all_payments_by_tenant(
#     session: SessionDep,
#     tenant: Annotated[Tenant, Depends(get_single_tenant)]
# ):
#     if not tenant:
...
#     statement = select(Payment).join(Invoice).join(MaintenanceBill).where(MaintenanceBill.tenant_id or Invoice.tenant_id == tenant.id)
...
async def get_all_maintenance_bills(
    ...
    statement = statement.options(selectinload(MaintenanceBill.payments), selectinload(MaintenanceBill.tenant))
